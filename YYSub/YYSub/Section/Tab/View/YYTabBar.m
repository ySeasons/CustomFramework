...
#define Titles @[@"sub首页",@"我的"]
#define Imgs @[@"tab1_nor",@"tab2_nor"]
#define SelImgs @[@"tab1_nor",@"tab2_nor"]


@implementation YYTabBar
+ (YYTabBar*)sharedInstace {
    static dispatch_once_t onceToken;
    static YYTabBar* tabBar;
    dispatch_once(&onceToken, ^{
        tabBar = [[YYTabBar alloc] initWithFrame:CGRectMake(0,0,KScreenWidth,TABBAR_HEIGHT)];
    });
    return tabBar;
}
...
- (void)setupTabBarView {
    ...
    for (int i = 0; i < Titles.count; i ++) {
        ...
        if (i == 2) {
            iconImgView.frame = CGRectMake(cWidth/2-22, -15, 44, 44);
        }
        if (i == 0) {
            [self updateState:0];
        }
    ...
    for (NSInteger i = 0; i < Titles.count; i ++) {
        ...
        if (index == i) {
            ...
            [UIView animateWithDuration:1 delay:0 usingSpringWithDamping:0.3 initialSpringVelocity:8 options:UIViewAnimationOptionCurveEaseInOut animations:^{
                if (i == 2) {
                    iconImgView.frame = CGRectMake(cWidth/2-23, -16, 46, 46);
                } else {
